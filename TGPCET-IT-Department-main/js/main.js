...
// === ENSURE CYBER CODING BACKGROUND IS LOADED ON ALL PAGES ===
(function ensureTechBackground() {
    if (typeof window !== 'undefined' && !window.__TECH_BG_INITIALIZED__) {
        if (!document.querySelector('script[src*="tech-bg.js"]')) {
            const script = document.createElement('script');
            script.src = 'js/tech-bg.js';
            script.async = true;
...
(function() {
    ...
    chatBtn.innerHTML = `
        <div style="position:relative;display:flex;align-items:center;justify-content:center;width:100%;height:100%;">
            <i class="fas fa-comment-dots" style="font-size:1.85rem;"></i>
            <span style="position:absolute;top:-4px;right:-4px;width:16px;height:16px;background:#10b981;border-radius:50%;border:3px solid #0f172a;box-shadow:0 0 10px #10b981;"></span>
        </div>
    `;
    chatBtn.title = "Ask TGPCET IT AI Assistant";
    ...
    // Create chatbot window (Fixed Large Size)
    ...
    chatWindow.style.cssText = `
        ...
        background: linear-gradient(165deg, rgba(15,23,42,0.98), rgba(24,33,56,0.98));
        backdrop-filter: blur(20px);
        -webkit-backdrop-filter: blur(20px);
        border-radius: 24px;
        box-shadow: 0 25px 70px rgba(0,0,0,0.65), 0 0 0 1px rgba(59,130,246,0.35);
        z-index: 997;
        display: none;
    ...
    // Inject chat animation styles
    if (!document.getElementById('it-chat-custom-style')) {
        const customStyle = document.createElement('style');
        customStyle.id = 'it-chat-custom-style';
        customStyle.textContent = `
            ...
            #chat-messages::-webkit-scrollbar-track { background: rgba(15,23,42,0.5); }
            #chat-messages::-webkit-scrollbar-thumb { background: rgba(59,130,246,0.4); border-radius: 3px; }
            #chat-messages::-webkit-scrollbar-thumb:hover { background: rgba(59,130,246,0.7); }
            .quick-chip {
    ...
    chatWindow.innerHTML = `
        <!-- Chat Header -->
        <div style="padding:1.25rem 1.4rem;background:linear-gradient(135deg, #1d4ed8, #7c3aed);color:white;display:flex;align-items:center;justify-content:space-between;border-bottom:1px solid rgba(255,255,255,0.15);">
            <div style="display:flex;align-items:center;gap:0.9rem;">
                <div style="width:48px;height:48px;background:rgba(255,255,255,0.18);border-radius:14px;display:flex;align-items:center;justify-content:center;box-shadow:inset 0 0 10px rgba(255,255,255,0.2);">
                    <i class="fas fa-brain" style="font-size:1.6rem;color:#f8fafc;"></i>
                </div>
                <div>
                    <div style="display:flex;align-items:center;gap:0.5rem;">
                        <h3 style="margin:0;font-size:1.15rem;font-weight:700;letter-spacing:0.3px;">TGPCET IT AI</h3>
                        <span style="font-size:0.68rem;background:rgba(16,185,129,0.25);border:1px solid #10b981;color:#6ee7b7;padding:2px 7px;border-radius:20px;font-weight:600;">ACTIVE</span>
                    </div>
                    <p style="margin:0.2rem 0 0;font-size:0.8rem;color:#cbd5e1;display:flex;align-items:center;gap:5px;">
                        <span style="width:7px;height:7px;border-radius:50%;background:#10b981;display:inline-block;box-shadow:0 0 6px #10b981;"></span>
                        Online Assistant • English & Hindi
                    </p>
                </div>
            </div>
            <div style="display:flex;align-items:center;gap:0.4rem;">
                <button id="clear-chat" title="Clear Chat" style="background:rgba(255,255,255,0.12);border:none;color:white;cursor:pointer;width:34px;height:34px;display:flex;align-items:center;justify-content:center;border-radius:10px;transition:all 0.2s;">
                    <i class="fas fa-rotate-right" style="font-size:0.9rem;"></i>
                </button>
                <button id="close-chat" title="Close" style="background:rgba(255,255,255,0.12);border:none;color:white;cursor:pointer;width:34px;height:34px;display:flex;align-items:center;justify-content:center;border-radius:10px;transition:all 0.2s;">
        ...
        <!-- Messages Area -->
        <div id="chat-messages" style="flex:1;padding:1.35rem;overflow-y:auto;display:flex;flex-direction:column;gap:1.1rem;">
            <!-- Welcome Message -->
            <div style="background:linear-gradient(145deg, rgba(30,41,59,0.85), rgba(15,23,42,0.9));padding:1.2rem;border-radius:18px;border-left:4px solid #3b82f6;border-top:1px solid rgba(59,130,246,0.25);border-right:1px solid rgba(59,130,246,0.15);border-bottom:1px solid rgba(59,130,246,0.15);">
                <div style="display:flex;align-items:center;gap:0.5rem;margin-bottom:0.6rem;">
                    <span style="font-size:1.3rem;">👋</span>
                    <strong style="color:#ffffff;font-size:1rem;">Welcome to TGPCET IT Assistant!</strong>
                </div>
                <p style="color:#cbd5e1;margin:0;font-size:0.92rem;line-height:1.6;">
                    Main TGPCET IT Department ka AI Assistant hoon. Aap mujhse Admissions, Placements, HOD info, Labs, Syllabus, ya <strong>TECH-XION 2.0</strong> ke baare me pooch sakte hain.
                </p>
            </div>

            <!-- Quick Topics Grid -->
            <div>
                <span style="font-size:0.78rem;font-weight:600;text-transform:uppercase;letter-spacing:0.6px;color:#94a3b8;display:block;margin-bottom:0.65rem;">Popular Questions:</span>
                <div style="display:grid;grid-template-columns:1fr 1fr;gap:0.55rem;">
                    <button class="quick-chip quick-reply" data-msg="IT Department ke HOD kaun hain aur contact details kya hain?">👨‍🏫 HOD Info</button>
                    <button class="quick-chip quick-reply" data-msg="IT branch me placement records aur top companies kaunsi hain?">💼 Placements</button>
                    <button class="quick-chip quick-reply" data-msg="TECH-XION 2.0 event ki dates aur competitions ke bare me batao">🏆 TECH-XION 2.0</button>
                    <button class="quick-chip quick-reply" data-msg="IT department me admission eligibility aur seats kitni hain?">🎓 Admissions</button>
                    <button class="quick-chip quick-reply" data-msg="IT Department me kitne labs aur facilities hain?">🖥️ Labs & Infra</button>
                    <button class="quick-chip quick-reply" data-msg="IT field me kaunse career domains best hain?">🚀 Career Guide</button>
                </div>
            </div>
        </div>
        
        <!-- Input Footer -->
        <div style="padding:1rem 1.25rem;background:rgba(15,23,42,0.95);border-top:1px solid rgba(59,130,246,0.25);">
            <div style="display:flex;align-items:center;gap:0.65rem;background:rgba(30,41,59,0.85);padding:0.35rem 0.5rem 0.35rem 1.1rem;border-radius:30px;border:1.5px solid rgba(59,130,246,0.35);transition:border-color 0.2s;" id="chat-input-wrapper">
                <input type="text" id="chat-input" placeholder="Type in Hindi or English..." style="flex:1;border:none;background:transparent;color:#f8fafc;font-size:0.95rem;outline:none;padding:0.45rem 0;">
                <button id="send-btn" title="Send message" style="width:42px;height:42px;border-radius:50%;background:linear-gradient(135deg,#3b82f6,#7c3aed);border:none;color:white;cursor:pointer;display:flex;align-items:center;justify-content:center;font-size:1.1rem;transition:all 0.2s;box-shadow:0 4px 12px rgba(59,130,246,0.4);">
                    <i class="fas fa-paper-plane"></i>
                </button>
            </div>
            <div style="text-align:center;margin-top:0.45rem;">
                <span style="font-size:0.75rem;color:#94a3b8;font-weight:500;">
                    ⚡ Powered by <strong style="color:#60a5fa;">Bhupesh Indurkar</strong> — Full Stack Developer
                </span>
            </div>
        </div>
    `;
    ...
    chatBtn.addEventListener('click', () => {
        ...
        chatBtn.style.transform = isVisible ? 'scale(1)' : 'scale(0.92)';
        if (!isVisible) {
            setTimeout(() => {
        ...
        clearBtn.addEventListener('click', () => {
            ...
            if (chatMessages) {
                chatMessages.innerHTML = `
                    <div style="background:linear-gradient(145deg, rgba(30,41,59,0.85), rgba(15,23,42,0.9));padding:1.2rem;border-radius:18px;border-left:4px solid #3b82f6;">
                        <p style="color:#cbd5e1;margin:0;font-size:0.92rem;line-height:1.6;">
    ...
    if (chatInputElem && chatInputWrapper) {
        chatInputElem.addEventListener('focus', () => {
            chatInputWrapper.style.borderColor = '#60a5fa';
            chatInputWrapper.style.boxShadow = '0 0 12px rgba(59,130,246,0.3)';
        });
        chatInputElem.addEventListener('blur', () => {
            chatInputWrapper.style.borderColor = 'rgba(59,130,246,0.35)';
            chatInputWrapper.style.boxShadow = 'none';
        });
    }

    // Hover effects on toggle button
    chatBtn.addEventListener('mouseenter', () => {
        chatBtn.style.transform = 'scale(1.08)';
        chatBtn.style.boxShadow = '0 16px 40px rgba(37,99,235,0.65)';
    });

    chatBtn.addEventListener('mouseleave', () => {
        chatBtn.style.transform = 'scale(1)';
        chatBtn.style.boxShadow = '0 12px 35px rgba(37,99,235,0.55)';
    });
    ...
    document.addEventListener('click', (e) => {
        const quickChip = e.target.closest('.quick-reply');
        if (quickChip) {
            const message = quickChip.getAttribute('data-msg');
    ...
    async function sendMessage(text) {
        ...
        // Try AI Chatbot API (OpenRouter / Gemini), fallback to local on failure
        let reply = '';
        const chatApiUrl = (window.location.hostname === 'localhost' || window.location.hostname === '127.0.0.1')
            ? 'http://localhost:5000/api/chat'
            : 'https://tgpcet-it-department.onrender.com/api/chat';
